...
#define MDC_MDIO_WRITE(preamableLength, phyID, regID, data)	ETH_WritePHYRegister(phyID, regID,data)
#define MDC_MDIO_READ(preamableLength, phyID, regID, pData) *pData = ETH_ReadPHYRegister(phyID,regID)
...
#define GPIO_DIRECTION_SET(gpioID, direction)		sim_smi_setDirection(gpioID,direction)
#define GPIO_DATA_SET(gpioID, data)							sim_smi_setData(gpioID, data)
#define GPIO_DATA_GET(gpioID, pData)						sim_smi_getData(gpioID, pData)
...
rtk_int32 smi_read(rtk_uint32 mAddrs, rtk_uint32 *rData)
{
    ...
    if(1 != MDC_MDIO_WRITE(MDC_MDIO_PREAMBLE_LEN, MDC_MDIO_PHY_ID, MDC_MDIO_CTRL0_REG, MDC_MDIO_ADDR_OP))
			return 1;

    /* Write address to register 23 */
    if(1 != MDC_MDIO_WRITE(MDC_MDIO_PREAMBLE_LEN, MDC_MDIO_PHY_ID, MDC_MDIO_ADDRESS_REG, mAddrs))
			return 1;

    /* Write read control code to register 21 */
    if(1 != MDC_MDIO_WRITE(MDC_MDIO_PREAMBLE_LEN, MDC_MDIO_PHY_ID, MDC_MDIO_CTRL1_REG, MDC_MDIO_READ_OP))
			return 1;
