...
static rtk_api_ret_t _rtk_cpu_enable_get(rtk_enable_t *pEnable)
{
    ...
    if ((retVal = rtl8367c_getAsicCputagEnable((rtk_uint32 *)pEnable)) != RT_ERR_OK)
        return retVal;
...
static rtk_api_ret_t _rtk_cpu_tagPort_get(rtk_port_t *pPort, rtk_cpu_insert_t *pMode)
{
    ...
    *pPort = rtk_switch_port_P2L_get(port);
    ...
    if ((retVal = rtl8367c_getAsicCputagInsertMode((rtk_uint32*)pMode)) != RT_ERR_OK)
        return retVal;
...
static rtk_api_ret_t _rtk_cpu_tagPosition_get(rtk_cpu_position_t *pPosition)
{
    ...
    if ((retVal = rtl8367c_getAsicCputagPosition((rtk_uint32*)pPosition)) != RT_ERR_OK)
        return retVal;
...
static rtk_api_ret_t _rtk_cpu_tagLength_get(rtk_cpu_tag_length_t *pLength)
{
    ...
    if ((retVal = rtl8367c_getAsicCputagMode((rtk_uint32 *)pLength)) != RT_ERR_OK)
        return retVal;
...
static rtk_api_ret_t _rtk_cpu_acceptLength_get(rtk_cpu_rx_length_t *pLength)
{
    ...
    if ((retVal = rtl8367c_getAsicCputagRxMinLength((rtk_uint32 *)pLength)) != RT_ERR_OK)
        return retVal;
