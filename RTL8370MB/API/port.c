...
static rtk_api_ret_t _rtk_port_phyStatus_get(rtk_port_t port, rtk_port_linkStatus_t *pLinkStatus, rtk_port_speed_t *pSpeed, rtk_port_duplex_t *pDuplex)
{
    ...
    if (phyData & (1<<2))
    {
        *pLinkStatus = static_cast<rtk_port_linkStatus_t>(1);

        /*check link speed*/
        *pSpeed = static_cast<rtk_port_speed_t>((phyData&0x0030) >> 4);

        /*check link duplex*/
        *pDuplex = static_cast<rtk_port_duplex_t>((phyData&0x0008) >> 3);
    }
    else
    {
        *pLinkStatus = static_cast<rtk_port_linkStatus_t>(0);
        *pSpeed = static_cast<rtk_port_speed_t>(0);
        *pDuplex = static_cast<rtk_port_duplex_t>(0);
    }
...
static rtk_api_ret_t _rtk_port_adminEnable_get(rtk_port_t port, rtk_enable_t *pEnable)
{
    ...
    if ((retVal = rtk_port_phyReg_get(port, static_cast<rtk_port_phy_reg_t>(PHY_CONTROL_REG), &data)) != RT_ERR_OK)
        return retVal;
...
static rtk_api_ret_t _rtk_port_phyEnableAll_set(rtk_enable_t enable)
{
    ...
    {
			if(rtk_switch_isUtpPort(static_cast<rtk_port_t>(port)) == RT_ERR_OK)
        {
            if ((retVal = rtk_port_phyReg_get(static_cast<rtk_port_t>(port), static_cast<rtk_port_phy_reg_t>(PHY_CONTROL_REG), &data)) != RT_ERR_OK)
                return retVal;
            ...
            {
                data |= 0x0800;	/*!< Restart auto-negotiation function */
            }

            if ((retVal = rtk_port_phyReg_set(static_cast<rtk_port_t>(port), static_cast<rtk_port_phy_reg_t>(PHY_CONTROL_REG), data)) != RT_ERR_OK)
                return retVal;
        }
    }
 ...
 * Description:
 *      Set all PHY enable status. Mainly reset all port register and set auto-negotiation
 * Input:
