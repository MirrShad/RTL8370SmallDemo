...
#define RTK_PORTMASK_IS_PORT_SET(__portmask__, __port__)    (((__portmask__).bits[0] & (0x00000001 << (int)__port__)) ? 1 : 0)
...
#define RTK_PORTMASK_SCAN(__portmask__, __port__)           for(__port__ = static_cast<rtk_port_t>(0); (int)__port__ < RTK_SWITCH_PORT_NUM; static_cast<rtk_port_t>(((int)__port__)++))  if(RTK_PORTMASK_IS_PORT_SET(__portmask__, __port__))
...
#define RTK_SCAN_ALL_LOG_PORT(__port__)                     for(__port__ = 0; __port__ < RTK_SWITCH_PORT_NUM; __port__++)  if( rtk_switch_logicalPortCheck(static_cast<rtk_port_t>(__port__)) == RT_ERR_OK)
