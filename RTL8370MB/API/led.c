...
static rtk_api_ret_t _rtk_led_modeForce_get(rtk_port_t port, rtk_led_group_t group, rtk_led_force_mode_t *pMode)
{
    ...
    if ((retVal = rtl8367c_getAsicForceLed(rtk_switch_port_L2P_get(port), group, (rtk_uint32*)pMode)) != RT_ERR_OK)
        return retVal;
...
static rtk_api_ret_t _rtk_led_blinkRate_get(rtk_led_blink_rate_t *pBlinkRate)
{
    ...
    if ((retVal = rtl8367c_getAsicLedBlinkRate((rtk_uint32*)pBlinkRate)) != RT_ERR_OK)
        return retVal;
...
static rtk_api_ret_t _rtk_led_groupConfig_get(rtk_led_group_t group, rtk_led_congig_t *pConfig)
{
    ...
    if ((retVal = rtl8367c_getAsicLedIndicateInfoConfig(group, (rtk_uint32*)pConfig)) != RT_ERR_OK)
        return retVal;
...
static rtk_api_ret_t _rtk_led_serialMode_get(rtk_led_active_t *pActive)
{
    ...
    if ((retVal = rtl8367c_getAsicLedSerialModeConfig((rtk_uint32*)pActive,&regData))!=RT_ERR_OK)
        return retVal;
...
static rtk_api_ret_t _rtk_led_OutputEnable_get(rtk_enable_t *pState)
{
    ...
    if ((retVal = rtl8367c_getAsicLedOutputEnable((rtk_uint32*)pState))!=RT_ERR_OK)
        return retVal;
